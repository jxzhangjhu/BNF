...
class BasePWL(torch.nn.Module):
    ...
    def slope_at(self, x):
        dx = 1e-3
        return -(self.forward(x)[0] - self.forward(x + dx)[0]) / dx
...
class BaseSlopedPWL(BasePWLX):
    ...
    def forward(self, x):
        ...
        return self.repack_input(cand, old_shape), selected_slopes
